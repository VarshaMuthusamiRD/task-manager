...
**A single static API key, not real auth.** The brief asked for "basic" API-key
authentication as a stretch goal, not a user/session system. A shared secret compared
against the `X-API-Key` header (`app/auth.py`) meets that bar without pulling in
OAuth libraries, password hashing, or a users table — all of which would be
over-engineering for a project with no concept of "who" is making a request.
...
|  +- TaskList.jsx       |  +header  |   +- require_api_key()   |--> 401 if bad/missing key
|  |   +- TaskItem.jsx   |           |  +- create_task_endpoint |
|  +- api.js (fetch      |           |  +- list_tasks_endpoint  |
|      wrapper, adds     |           |  +- get_task_endpoint    |
|      X-API-Key)        |           |  +- update_task_endpoint |
...
status dropdown) calls a function in `api.js`, which sends a `fetch` request to the
backend with the `X-API-Key` header attached. FastAPI's dependency system runs
`require_api_key` before the route handler — a bad or missing key short-circuits to
`401` and the handler never runs. If the key is valid, the handler calls into
`crud.py`, which runs SQL against the connection it's given and returns a plain
`dict`; FastAPI serializes that against the `TaskOut` Pydantic model into JSON.
...
slice's endpoints (create/list, retrieve/update/delete, auth) plus one dedicated
persistence test — 16 tests total, covering the happy path and at least the two
...
configuration. The API key is a single shared secret suitable for one trusted client,
